...
# this class is a child class of sensor and models the radar, as of now its just a copy of the lidar class
class Radar(Sensor):
    def __init__(self, position, fov_h, fov_v, detection_range, min_range, name):
        super().__init__(position, name, "radar")
        self.fov_h = fov_h
        self.fov_v = fov_v
    ...
    # private function to compute the points inside the fov of the radar. takes a point matrix of shape nx3 as input
    ...
    # function that is called by the main program to compute the radar coverage and set the metrics
    ...
    def to_yaml(self):
        """
        Convert the Radar object to a dictionary representation for YAML serialization.
        :return: Dictionary representation of the Radar object.
        """
        return {
            'type': 'Radar',
            'position': {
